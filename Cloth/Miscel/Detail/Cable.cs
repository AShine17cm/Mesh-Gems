...
namespace Mg.Games
{
    ...
    public class Cable
    {
        ...
        [Header("末端球体")] public float radius = 0.5f;
        ...
        public void Init(GameObject gameObject, Vector3 dirForce)
        {
            ...
            rads = new float[countNode];
            intersects = new float[countNode];
            //节点 距离
            ...
            for (int i = countNode - 1; i >= 0; i -= 1)
            {
                ...
                t = 1f - step * i;
                rads[i] = radius * curveRadius.Evaluate(t);
                //rads[i] = radius;
            }
            switch (bump)
            {
            ...
            InitMesh(countNode, countSeg, gameObject, dirForce);
        }
        public void UpdateLink(float tick, Vector3 dirForce,Vector3 delta,float amtMimic)
        {
            ...
            //7.9
            //Vector3 ofPos = trCable.position;
            //Vector3 delta = ofPos - nodes[0];
            //nodes[0] = ofPos;
            //7.9
            //模拟比重 7.15
            Vector3 pos = trCable.position;
            Vector3 delta0 = pos - nodes[0];
        ...
        void Refresh(Vector3 delta, Vector3 dirForce, float tick)
        {
            RaycastHit hitInfo;
            intersects[0] = 0;
            int countSect = 0;//相交 计数

            Vector3 offsetH = Vector3.up * 100f;
            Vector3 virPos;
            Vector3 virDir;
            Vector3 conPos;
            for (int i = 1; i < countNode; i++)
            {
                ...
                float rad = rads[i];
                float intersect = 0;
                if (Physics.Raycast(conPos + offsetH, Vector3.down, out hitInfo, 100f + rad, mask))
                {
                    intersect = 100f + rad - hitInfo.distance;
                    countSect += 1;
                }
            ...
            {
                float inter = 0;
                int countSum = 0;   //卷积 计数
                for (int i = countNode - 1; i >= 1; i -= 1)
                {
                    int sub = countSum - countSect;
                    if (sub >= 0)
                    {
                        inter -= intersects[i + countSect];
                    }
                    countSum += 1;
                    inter += intersects[i];
                    nodes[i] = nodes[i] - inter / (Mathf.Min(countSect, countSum)) * dirForce;
                }
            }
        }
        ...
        Mesh mesh;
        Bounds bd;
        void InitMesh(int countNode, int countSeg, GameObject gameObject, Vector3 dirForce)
        {
        ...
        public void OnDrawGizmos()
        {
            ...
            Handles.DrawWireCube(nodes[countNode - 1],Vector3.one*radius);
        }
#endif
    }
    public enum TailBumpEnum
    {
        BumpUp = 1,
        BumpDown = 2,  //凸起更靠近末端
    }
